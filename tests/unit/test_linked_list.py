...
def test_insert_head(sorted_list):
    ...
    assert sorted_list.to_list() == [0, 0, 1, 2, 3, 4]


def test_insert_tail(sorted_list):
    sorted_list.insert_tail(0)
    assert sorted_list.to_list() == [0, 1, 2, 3, 4, 0]


def test_insert_nth(sorted_list):
    sorted_list.insert_nth(0, 3)
    assert sorted_list.to_list() == [0, 1, 2, 0, 3, 4]
...
def test_iter(sorted_list):
    gen = sorted_list.iter()
    li = [i.content for i in gen]
    assert li == [0, 1, 2, 3, 4]


def test_serialization(sorted_list):
    assert str(sorted_list) == "[0, 1, 2, 3, 4]"
